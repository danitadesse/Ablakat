...
function startCountdowns() {
    const birthdayDate = new Date("2024-03-25").getTime();
...
function checkAnswer(questionNumber) {
    ...
    const resultElement = questionDiv.querySelector(`p[id="result${questionNumber}"]`);
    ...
    if (questionNumber === 1) {
        ...
        correctAnswer = "ablakat";
    } else if (questionNumber === 3) {
        correctAnswer = "history book";
    } else if (questionNumber === 4) {
        correctAnswer = "friends";
    } else if (questionNumber === 5) {
        correctAnswer = "daniel tadesse admasu";
    }
    ...
    // Save answer and result to localStorage
    localStorage.setItem(`answer${questionNumber}`, answer);
    localStorage.setItem(`result${questionNumber}`, resultElement.textContent);
    inputElement.dataset.answer = answer;
    resultElement.dataset.result = resultElement.textContent;
}
...
function loadSavedAnswers() {
    for (let i = 1; i <= 5; i++) {
        ...
        const savedResult = localStorage.getItem(`result${i}`);
        const questionDiv = document.querySelector(`[data-question="${i}"]`);
        const inputElement = questionDiv.querySelector(`input`);
        const resultElement = questionDiv.querySelector(`p[id="result${i}"]`);

        if (savedAnswer) {
            inputElement.value = savedAnswer;
            inputElement.dataset.answer = savedAnswer;
        }
        if (savedResult) {
            resultElement.textContent = savedResult;
            resultElement.dataset.result = savedResult;
            if (savedResult.includes("Correct")) {
                resultElement.style.color = "green";
            } else if (savedResult.includes("Incorrect")) {
                resultElement.style.color = "red";
            }
        }
    }
...
// Call loadSavedAnswers when the quiz page is shown
function showQuiz() {
    document.getElementById("birthday-page").style.display = "none";
    document.getElementById("quiz-page").style.display = "block";
    loadSavedAnswers();
